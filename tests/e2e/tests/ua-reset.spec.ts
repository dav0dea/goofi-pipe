...
// no such rule — they rendered in the browser's generic monospace, which is also why ~40 product
// rules restate `font-family: var(--font-mono)` on elements that should simply inherit it.

// The probe is the one `<code>` in the tree that declares no font of its own: Panel's unknown-type
// fallback, which a `.gfi` layout naming a retired panel reaches for real.
test('a <code> with no rule of its own still renders in the app mono face', async ({ page }) => {
	await page.goto('/');
	...
	try {
		...
		expect(font, 'the UA reset hands <code> back to the inherited app face').toContain(
			'JetBrains Mono'
		);
	} finally {
...
// that sizes one of these elements still wins. This is what makes the ~5 now-redundant
// `font-family: var(--font-mono)` restatements on them deletable — the family comes back by
// inheritance, the RUNG does not come from here. The console row is the reachable one.
