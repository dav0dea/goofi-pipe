...
pub static REGISTRY: &[Op] = &[
    ...
    Op { name: "set_viewpoint", surface: ControlOnly, writes: true, args: "viewpoint:json!",
         ...
         result: "{ok: true}" },
    Op { name: "inspect_layout", surface: Mcp, writes: false, args: "page:string",
         doc: "The arrangement as a tree: every page, split and panel with its id, order and share of its parent. How a caller discovers a page's name and the split id it names as a move target. `page` narrows it to one page; no arg = all of them.",
         result: "{text: string}" },
