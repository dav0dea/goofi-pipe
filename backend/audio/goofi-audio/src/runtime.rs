...
pub struct Slot {
    ...
    /// What each scalar param's region holds; NaN until the first block fills it.
    pub last: Vec<f64>,
}
...
impl Runtime {
    ...
    fn apply(&mut self, msg: Msg) {
        let retired = match msg {
            ...
            Msg::Plan { plan, arena } => {
                // A fresh arena holds nothing: every scalar region is filled again at the next block.
                for slot in self.slab.iter_mut().flatten() {
                    slot.last.fill(f64::NAN);
                }
    ...
    pub fn render_block(&mut self) {
            ...
            for src in stage.params.iter().chain(&stage.ins) {
                match src {
                    Source::Scalar { at, param } => {
                        let v = f64::from_bits(slot.params[*param].load(Ordering::Relaxed));
                        if v != slot.last[*param] {
                            unsafe { region_mut(base, len, *at, 1) }.fill(v as f32);
                            slot.last[*param] = v;
                        }
                    }
                    Source::Sum { at, channels, parts } => {
                        ...
                        for (part, pc) in parts {
                            let src = unsafe { region(base, len, *part, *pc) };
                            for c in 0..*channels as usize {
                                let sc = c.min(*pc as usize - 1);
                                for i in 0..BLOCK {
                                    dst[c * BLOCK + i] += src[sc * BLOCK + i];
                                }
                            }
                        }
                    }
        ...
        let (at, channels) = match &self.plan.output {
