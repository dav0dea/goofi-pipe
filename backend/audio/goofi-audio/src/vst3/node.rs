...
impl Live {
    ...
    fn retune(&mut self, rate: f64) -> Result<(), String> {
        ...
        unsafe {
            ...
            // OPTIONAL in the SDK, unlike the two above: a plugin that does not distinguish
            // processing from active answers kNotImplemented, which is an answer and not a
            // refusal. Treating it as one left Graindad permanently in error and silent.
        ...
        // A TRANSPORT, because a zeroed one is a stopped one. Left at zero the plugin is told the
        // host is not playing, at no tempo, at no position — and every tempo-synced engine
        // correctly does nothing: Graindad's grains, a synced delay, an LFO on note divisions. It
        // cost a granular plugin that answered every parameter and emitted pure silence.
...
unsafe fn pair(
    ...
    // The controller is told what the processor holds, which is what it builds its own state from.
