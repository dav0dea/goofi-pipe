...
impl IHostApplicationTrait for Host {
    ...
    /// The host's message allocator: a connected component and controller ask here, and a host that
    /// answers null hands back a pointer the plugin then dereferences.
    unsafe fn createInstance(&self, cid: *mut TUID, _iid: *mut TUID, obj: *mut *mut c_void) -> tresult {
        // A match arm on these consts trips the default `non_upper_case_globals` on their own
        // names, which CI's blanket `-D warnings` makes an error; so compared, not matched.
        let want = *cid;
