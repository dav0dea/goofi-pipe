...
class EdgeDetector(Node):
    ...
    def process(self, image: Data):
        ...
        # cv2 wants 8-bit; coerce the float [0,1] input internally (this stays
        # inside the node and never changes its float output dtype).
        ...
        # Emit a float [0,1] edge map (the node's output stays pure float).
