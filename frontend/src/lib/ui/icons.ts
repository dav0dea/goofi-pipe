/**
 * The app's OWN icon geometry — Lucide (https://lucide.dev), hand-vendored. The panel chrome
 * vendors the glyphs IT draws (`$lib/workspace/ui/icons`); this is everything else, handed to the
 * one renderer at startup with `registerIcons`. Two tables, one owner each, no glyph in both.
 *
 ...
 *
 * WHY VENDORED. A score of icons is not an npm dependency's worth of surface, and every icon the
 ...
 * `icons.test.ts` reads the bytes of BOTH tables and holds all of that to them.
 */
import { registerIcons, type ChromeIconName } from '$lib/workspace/ui/icons';
export const ICONS = {
	activity:
...
/** Every icon the app can draw: its own, plus the panel chrome's — one renderer draws both, so a
 * caller that names either is naming something real. The renderer itself takes a plain string (an
 * app's set is not enumerable from inside it), so this type is what a table of the app's own —
 * a panel type, a menu row — still declares against. */
export type IconName = keyof typeof ICONS | ChromeIconName;

// Handed to the one renderer HERE, by the table itself, rather than by whoever mounts an app root:
// the gallery route is a second root, a third is a `<script>` away, and a root that forgot draws
// every app icon as an empty box. Importing the table is what needs it, so importing it is what
// does it.
registerIcons(ICONS);
